...
Pascal VOC 2012 es dataset que contiene 17125 imágenes y una anotacion para cada una de ellas donde se identifican objetos comprendidos dentro de 20 categorías: personas, animales, vehículos e interiores. Es ampliamente usado para evaluar modelos de detección y segmentación de objetos.

### 1.1 Estadísticas básicas

Al explorar el dataset encontramos que las imágenes tienen dimensiones variables pero con una fuerte concentración alrededor de 500×375 píxeles y su transpuesta. Todas las imágenes son RGB (3 canales).
...
| Desbalance máx/mín entre clases | 28.2x  (person: 9583 vs cow: 340) |
...
El scatter de dimensiones muestra claramente dos clusters: imágenes en orientación paisaje (500×375) y en orientación retrato (375×500). Hay pocos casos fuera de esas dos resoluciones dominantes. El histograma de tamaño de archivo muestra una distribución sesgada a la derecha, con la mayoría de las imágenes por debajo de los 200 KB.
...
Cada imagen del dataset fue codificada con CLIP como un vector de 512 dimensiones y almacenada en un índice FAISS para permitir búsqueda eficiente por similitud.
...
### 2.3 Componente agéntico — LLMs livianos
...
### 2.5 Verificación de integridad
...
Esta decisión de diseño evita que una expansión semánticamente alejada contamine los resultados. Resultó importante en práctica porque Flan-T5 a veces genera alternativas demasiado genéricas o que derivan del tema original.
...
Para queries que involucran múltiples objetos simultáneos, construimos el ground truth como la intersección de las imágenes VOC que contienen todos los componentes de la query. Por ejemplo, para "person riding a horse" el GT son las imágenes anotadas tanto como `person` como `horse`.
...
Para el problema de las negaciones, una mejora concreta sería anotar manualmente un subconjunto de imágenes VOC con queries del estilo "X sin Y" y usarlo como conjunto de evaluación, lo que permitiría medir el impacto real de cada estrategia con métricas cuantitativas en lugar de métricas indirectas.
